...
/* Real photo with automatic emoji fallback if the hotlinked photo ever fails to load */
function photoInner(id, emoji) {
  const url = imgFor(id);
  if (!url) return emoji;
  return `<img src="${url}" alt="" loading="lazy" onerror="this.parentElement.classList.add('img-fallback')"><span class="fallback-emoji">${emoji}</span>`;
}

function restaurantCardHTML(r) {
  return `
  <article class="r-card" data-action="open-restaurant" data-id="${r.id}">
    <div class="r-banner photo-box">${photoInner(r.id, r.banner)}<span class="r-price">${priceDots(r.priceTier)}</span></div>
    <div class="r-body">
      <h3>${r.name}</h3>
...
function foodCardHTML(item) {
  ...
  <article class="f-card">
    ...
    <div class="f-img photo-box" data-action="open-product" data-id="${item.id}">${photoInner(item.id, item.emoji)}</div>
    <div class="f-body">
...
function renderMenuPage(restaurantId) {
  ...
  document.getElementById("restHeroBox").innerHTML = `
    <div class="emoji-huge photo-box" style="width:110px; height:110px; border-radius:var(--radius); flex-shrink:0;">${photoInner(r.id, r.banner)}</div>
...
function renderProductPage(itemId) {
  ...
  document.getElementById("productDetailBox").innerHTML = `
    <div>
      <div class="product-photo photo-box">${photoInner(item.id, item.emoji)}</div>
...
function cartRowHTML(c) {
  ...
  return `
  <div class="cart-row">
    <div class="f-img-sm photo-box">${photoInner(item.id, item.emoji)}</div>
